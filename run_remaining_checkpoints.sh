...
# HVA-X Analysis Script for Remaining Checkpoint Folders
# Runs Phase 2A (direct mode), 2B, and 3 for remaining checkpoints (excluding steps_00199992)
...
SKIP_CHECKPOINT="steps_00199992"  # Skip this already completed checkpoint
...
run_checkpoint_analysis() {
    ...
    local checkpoint_start=$(date +%s)  # Track start time
...
# Main execution
main() {
    print_status "Starting HVA-X Analysis for Remaining Checkpoints"
    print_status "Checkpoint directory: $CHECKPOINT_DIR"
    print_status "Results will be saved to: $RESULTS_BASE_DIR"
    print_warning "Skipping already completed checkpoint: $SKIP_CHECKPOINT"
    echo "========================================"
    ...
    # Find all checkpoint subdirectories except the one we want to skip
    all_checkpoints=($(find "$CHECKPOINT_DIR" -maxdepth 1 -type d -name "steps_*" | sort))
    checkpoints=()
    
    for checkpoint in "${all_checkpoints[@]}"; do
        checkpoint_name=$(get_checkpoint_name "$checkpoint")
        if [ "$checkpoint_name" != "$SKIP_CHECKPOINT" ]; then
            checkpoints+=("$checkpoint")
        fi
    done
    
    if [ ${#checkpoints[@]} -eq 0 ]; then
        print_error "No remaining checkpoint directories found in $CHECKPOINT_DIR"
        exit 1
    fi
    
    print_status "Found ${#checkpoints[@]} remaining checkpoint directories to process:"
    for checkpoint in "${checkpoints[@]}"; do
        print_status "  - $(get_checkpoint_name "$checkpoint")"
    done
    print_status "Total checkpoints found: ${#all_checkpoints[@]} (skipping 1)"
    echo "========================================"
    
    # Process each remaining checkpoint
    success_count=0
    failure_count=0
    ...
    echo "========================================"
    print_status "🎯 HVA-X ANALYSIS COMPLETE (REMAINING CHECKPOINTS)"
    print_status "Remaining checkpoints processed: ${#checkpoints[@]}"
    ...
    # List all generated reports (including the previously completed one)
    echo ""
    print_status "All Generated Reports:"
    find "$RESULTS_BASE_DIR" -name "*_report_*.md" | sort | while read -r report; do
        rel_path=$(realpath --relative-to="." "$report")
        checkpoint_name=$(echo "$rel_path" | sed 's/.*\/\(steps_[0-9]*\)\/.*/\1/')
        if [ "$checkpoint_name" == "$SKIP_CHECKPOINT" ]; then
            print_success "  📄 $rel_path (previously completed)"
        else
            print_success "  📄 $rel_path (newly generated)"
        fi
    done
    
    if [ $failure_count -eq 0 ]; then
        print_success "🎉 All remaining checkpoints processed successfully!"
        print_success "🎯 Complete analysis now available for all ${#all_checkpoints[@]} checkpoints!"
        exit 0
    else
